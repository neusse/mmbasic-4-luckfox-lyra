param(
  ...
  [string]$LocalPng = "$env:TEMP\picocalc-captures\mmbasic-fbdev-capture.png",
...
$repoBasic = Join-Path $repoRoot 'tests\picocalc\tst_picocalc_gfx_framebuffer_capture.bas'
$bundleBasic = Join-Path $repoRoot 'share\tests\picocalc\tst_picocalc_gfx_framebuffer_capture.bas'
$basic = if (Test-Path -LiteralPath $repoBasic) { $repoBasic } else { $bundleBasic }
...
$remotePng = '/tmp/mmbasic-fbdev-capture.png'
$remoteLog = '/tmp/mmbasic-fbdev-test.log'
$remoteRc = '/tmp/mmbasic-fbdev-test.rc'
$remoteShotLog = '/tmp/mmbasic-fbdev-shot.log'
$remoteScript = '/tmp/verify-mmbasic-fbdev.sh'
...
$tempScript = Join-Path $env:TEMP 'verify-mmbasic-fbdev.sh'
...
if ($rcLine -ne 'mmbasic_rc:0') {
  throw "fbdev BASIC test failed: $rcLine"
}
...
Write-Output "fbdev capture verified: $localPngPath"
